...
export default function PracticeSession() {
    ...
    useEffect(() => {
        ...
        const fetchCompletedSessions = async () => {
            try {
                ...
                const data = await response.json();
                setCompletedSessions(data.sessionIds.map(id => id._id.toString()));
            } catch (error) {
    ...
    const completedDays = uniqueDays.filter(day => {
        const session = sessions.find(s => s.day === day && s.instrument === selectedInstrument);
        return session && completedSessions.includes(session._id.toString());
    }).length;
    ...
    const isDayAccessible = (day) => {
        ...
        const previousSession = sessions.find(s => s.day === previousDay && s.instrument === selectedInstrument);
        return previousSession && completedSessions.includes(previousSession._id.toString());
    };
            ...
            <div className="relative flex flex-1">
                ...
                <main className="flex-1 p-6 flex justify-center items-start mt-4">
                    <div className="bg-white bg-opacity-60 backdrop-blur-lg dark:bg-gray-800 dark:bg-opacity-80 rounded-3xl shadow-lg p-8 w-full max-w-7xl h-[85vh]">
                        <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">Practice Sessions for {selectedInstrument}</h2>
                        ...
                        <div className="overflow-y-auto flex-grow p-2 space-y-4">
                            {uniqueDays.length === 0 ? (
                                ...
                                uniqueDays.map((day) => {
                                    const session = sessions.find(s => s.day === day && s.instrument === selectedInstrument);
                                    const isCompleted = session && completedSessions.includes(session._id.toString());
                        ...
                        <div className="mt-4">
                            ...
                            <p className="text-left text-sm font-medium mt-1 text-gray-700 dark:text-gray-300">{Math.round(completionPercentage)}% Completed</p>
                        </div>
                    </div>
                </main>
