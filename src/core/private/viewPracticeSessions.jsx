...
const ViewPracticeSessions = () => {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`http://localhost:3000/api/sessions/${editSession._id}`, formData, {
                headers: {
    ...
    const renderMedia = (file) => {
            ...
            return (
                <div className="w-full max-h-[200px] flex justify-center">
                    <img
                        ...
                        onError={(e) => {
                            e.target.outerHTML = `<a href="http://localhost:3000/${file}" target="_blank" class="text-blue-500 dark:text-blue-400 underline">${file}</a>`;
                        }}
                    />
                </div>
            );
        ...
        return (
            <a href={`http://localhost:3000/${file}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 dark:text-blue-400 underline">
                {file}
            </a>
        );
    };
