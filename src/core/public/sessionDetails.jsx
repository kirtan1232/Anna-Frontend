...
export default function SessionDetails() {
    ...
    useEffect(() => {
        ...
        const fetchSessions = async () => {
            try {
                ...
                if (filtered.length > 0) {
                    ...
                    fetchCompletionStatus(filtered[0]._id);
                }
            } catch (error) {
        ...
        const fetchCompletionStatus = async (sessionId) => {
            try {
                ...
                const data = await response.json();
                const isCompleted = data.sessionIds.some(id => id._id.toString() === sessionId.toString());
    ...
    const markAsComplete = async () => {
        ...
        try {
            const response = await fetch("http://localhost:3000/api/completed-sessions/toggle", {
                ...
                body: JSON.stringify({ sessionId: sessions[0]._id }),
            });
    ...
    return (
        <div className={`h-screen flex flex-col ${theme === 'light' ? 'bg-gradient-to-br from-purple-100 to-blue-100' : 'bg-gradient-to-br from-gray-900 to-gray-800'} fallback:bg-gray-900`}>
            <div className="relative flex flex-1">
                ...
                <main className="flex-1 p-12 flex flex-col items-start ml-4">
                    <div className="bg-white bg-opacity-60 backdrop-blur-lg dark:bg-gray-800 dark:bg-opacity-80 rounded-3xl shadow-lg p-8 w-full max-w-7xl h-[85vh]">
                        <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">Sessions for {day} - {instrument}</h2>
                                            ...
                                            <button
                                                ...
                                                className={`mt-4 text-white px-4 py-2 rounded-md transition-all duration-500 ${
                                                    canMarkComplete
                                                        ...
                                                        : 'bg-[#CDC2F5] dark:bg-gray-600 cursor-not-allowed'
                                                }`}
                                            >
