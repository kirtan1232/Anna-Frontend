...
const ViewLessons = () => {
    ...
    useEffect(() => {
        const fetchQuizzes = async () => {
            setLoading(true);
            try {
                const response = await axios.get('http://localhost:3000/api/quiz/getAllQuizzes', {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:3000/api/quiz/updatequiz/${editQuiz._id}`, formData, {
                headers: {
    ...
    const handleDeleteQuiz = async (quizId) => {
        ...
        try {
            await axios.delete(`http://localhost:3000/api/quiz/deletequiz/${quizId}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
                                                ...
                                                {quiz.quizzes && quiz.quizzes.length > 0 ? (
                                                    quiz.quizzes.map((q, qIndex) => (
                                                        <div key={qIndex} className="mb-4">
                                                            ...
                                                            {q.chordDiagram && (
                                                                <img
                                                                    src={`http://localhost:3000/uploads/${q.chordDiagram}`}
                                                                    alt="Chord Diagram"
